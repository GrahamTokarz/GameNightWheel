...
for i in range(len(wheel_files)):
    with open(f"wackbox\{wheel_files[i]}.json") as f:
        data = json.load(f)
        wheelLabels[wheel_names[i]] = [data[str(i)] for i in range(1, len(data)+1)]

wheel_options = random.sample(wheelLabels['Jackbox Wheel'], k=len(wheelLabels['Jackbox Wheel']))
...
changeWheel(wheelLabels['Jackbox Wheel'])
...
neon_colors = ["#FF00FF","#00FFFF","#FFFF00","#FF4500","#00FF00","#1E90FF"]
...
def draw_wheel(surface, angle):
    global lastWheelOptions, precomputed_wheel, unmodifiedLastSpinResult, color_shift_index
    ...
    color_shift_index = (color_shift_index + 1) % len(neon_colors)
    ...
    for i, slice_data in enumerate(precomputed_wheel):
        ...
        for p in slice_data["points"]:
            ...
            # gentle wobble
            dx += math.sin(time.time()*2 + i) * 2
            dy += math.cos(time.time()*2 + i) * 2
            x = wheel_pos.x + dx * cos_r - dy * sin_r
            y = wheel_pos.y + dx * sin_r + dy * cos_r
            rotated_points.append((x, y))

        # smooth color cycling
        pygame.draw.polygon(surface, neon_colors[(i + color_shift_index) % len(neon_colors)], rotated_points)

        # label sway
        sway_angle = math.sin(time.time()*2 + i) * 10  # ±10°
        mid_angle = slice_data["mid_angle"] + rad
        label_x = wheel_pos.x + (wheel_radius * 0.7 * math.cos(mid_angle))
        label_y = wheel_pos.y + (wheel_radius * 0.7 * math.sin(mid_angle))
        rotated_label = pygame.transform.rotate(slice_data["label_surf"], -math.degrees(mid_angle) + sway_angle)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if milkTime and accept_rect.collidepoint(event.pos):
                milkTime = False
