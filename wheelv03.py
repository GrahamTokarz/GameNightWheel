...
allSlices = {}
with open("wheelsV0.3/allSlices.json") as f:
    allSlices = json.load(f)
    # print(allSlices)
    
allPastSlices = {}
with open("wheelsV0.3/allPastSlices.json") as f:
...
for key in WHEEL_LOADER:
    with open(f"wheelsV0.3/{WHEEL_LOADER[key]}.json") as f:
...
for key in QUEUED_WHEELS:
    with open(f"wheelsV0.3/{QUEUED_WHEELS[key]["name"]}.json") as f:
        wheelProperties = json.load(f)
        merged_dict = []
        wheel_builder = []
        ...
        if event.type == pygame.QUIT:
            ...
            if needAccept and accept_rect.collidepoint(event.pos):
                ...
                if (winning_op["removeFromWheel"] == 1):
                    ...
                    jackboxWheels = ["Low Rated Jackbox Wheel", "Mid Rated Jackbox Wheel", "High Rated Jackbox Wheel", "Jackbox Arcade Games Wheel", "Jackbox Drawing Games Wheel", "Jackbox Prompt Games Wheel", "Jackbox Trivia Games Wheel"]
                    if (currentWheel in jackboxWheels):
                        for jbw in jackboxWheels:
                            for i in range(len(wheelLabels[jbw])):
                                if (wheelLabels[jbw][i]["id"] == winning_op["id"]):
                                    del wheelLabels[jbw][i]
                                    break
                    jackIDs = {
                        "WHJackLow": "Low Rated Jackbox Wheel",
                        "WHJackMid": "Mid Rated Jackbox Wheel",
                        "WHJackHigh": "High Rated Jackbox Wheel",
                        "WHJackArcade": "Jackbox Arcade Games Wheel",
                        "WHJackDraw": "Jackbox Drawing Games Wheel",
                        "WHJackPrompt": "Jackbox Prompt Games Wheel",
                        "WHJackTrivia": "Jackbox Trivia Games Wheel"
                    }
                    for i in range(7):
                        JBwheelName = "Jackbox Ratings Wheel" if i < 3 else "Jackbox Types Wheel"
                        JBwheelIdx = i if i < 3 else i - 3
                        wheelLabels[JBwheelName][JBwheelIdx]["sizeMult"] = len(wheelLabels[jackIDs[wheelLabels[JBwheelName][JBwheelIdx]["id"]]])
                        wheelLabels[JBwheelName][JBwheelIdx]["winMult"] = len(wheelLabels[jackIDs[wheelLabels[JBwheelName][JBwheelIdx]["id"]]])
                else:
                    if (winning_op["removeFromWheel"] > 1):
                ...
                if (wheelWheelSpins == 2 and not wheelWheelV2):
                    wheelWheelV2 = True
                    addPremadeSlice({
                        "label": "America Wheel",
                        "full": None,
                        ...
                        "winMult": 1,
                        "removeFromWheel": 0,
                        "goToWheel": "America Wheel",
                        "specialEffect": None,
                        "specialColor": "#D91012",
                        "id": "WHMurica"                          
                    })
                    
                if (wheelWheelSpins == 4 and wheelWheelV2):
                    wheelWheelV2 = False
                    addPremadeSlice({
                        "label": "Everyone's Actual Favorite Wheel",
                        "full": None,
                        "sizeMult": 1,
                        "winMult": 1,
                        "removeFromWheel": 0,
                        "goToWheel": "Everyone's Actual Favorite Wheel",
                        "specialEffect": None,
                        "specialColor": "#DEC93E",
                        "id": "WHEAFW"                          
                    })
